...
show_usage() {
    echo "Load Balancer Management Script"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  $0 --deploy   # Deploy load balancer"
    echo "  $0 --cleanup  # Remove load balancer"
    exit 1
}
...
deploy() {
    echo "🚀 Deploying shared load balancer..."
    echo "📦 Installing ALB controller and IngressClasses..."
    helm repo add eks https://aws.github.io/eks-charts
    helm repo update
    helm dependency build charts/load-balancer/
    helm upgrade --install load-balancer charts/load-balancer/ \
        --namespace kube-system
    
    # Wait for AWS Load Balancer Controller to be ready
    echo "⏳ Waiting for AWS Load Balancer Controller to be ready..."
    kubectl wait --for=condition=available --timeout=60s deployment/load-balancer-aws-load-balancer-controller -n kube-system
...
# Function to cleanup load balancer
cleanup() {
    echo "🧹 Cleaning up shared load balancer..."
    helm uninstall load-balancer -n kube-system --ignore-not-found || true
