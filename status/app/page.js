    ...
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-3">
          ...
          <p className="text-sm">Fetching real-time metrics from Prometheus...</p>
        </div>
      );
    }
  ...
  return (
    <div className="min-h-screen flex flex-col justify-between bg-background text-foreground">
      <main className="max-w-4xl w-full mx-auto px-4 py-12">
        ...
        <Card className="mt-8 border-dashed">
          <CardContent className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h4 className="text-sm font-semibold">Live Prometheus Telemetry</h4>
              <p className="text-xs text-muted-foreground mt-0.5">
                Connected to Prometheus on localhost:9090. Sampling 5-minute health checks over the past 30 days.
                {lastUpdated && (
      ...
      <footer className="border-t border-border py-6 text-center text-xs text-muted-foreground">
        &copy; {new Date().getFullYear()} Public AI. All rights reserved.
      </footer>
    </div>
  );
}
